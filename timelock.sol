...
contract ChickenTimeLock {
    ...
    function queueUpdateEmissions(uint256 ChickenPerBlock) external onlyOwner returns (bytes32 txId) {
        string memory _func = "updateEmissionRate(uint256)";
        uint _timestamp = block.timestamp + TIME_DELAY;
        bytes memory _data = abi.encodeWithSignature(_func, ChickenPerBlock);
        txId = getTxId(_func, _data, _timestamp);
        if (queued[txId]) {
            revert AlreadyQueuedError(txId);
        }
